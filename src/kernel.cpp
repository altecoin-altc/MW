...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZMWStake(spend));
    } else {
        ...
        CMWStake* MWInput = new CMWStake();
        MWInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(MWInput);
    }
