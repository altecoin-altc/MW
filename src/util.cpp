...
int nAnonymizeMasterWinAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "masterwin" is a composite category enabling all MasterWin-related debug output
            if (ptrCategory->count(string("masterwin"))) {
...
#else
    const char* pszModule = "masterwin";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterWin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterWin
// Mac: ~/Library/Application Support/MasterWin
// Unix: ~/.masterwin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterWin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "masterwin.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty masterwin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override masterwin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "masterwind.pid"));
