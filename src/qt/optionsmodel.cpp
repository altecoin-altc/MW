...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MW);
    ...
    if (!settings.contains("nAnonymizeMasterWinAmount"))
        settings.setValue("nAnonymizeMasterWinAmount", 1000);

    nAnonymizeMasterWinAmount = settings.value("nAnonymizeMasterWinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMasterWinAmount"))
        SoftSetArg("-anonymizemasterwinamount", settings.value("nAnonymizeMasterWinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in masterwin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterWinAmount:
            return QVariant(nAnonymizeMasterWinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterWinAmount:
            nAnonymizeMasterWinAmount = value.toInt();
            settings.setValue("nAnonymizeMasterWinAmount", nAnonymizeMasterWinAmount);
            emit anonymizeMasterWinAmountChanged(nAnonymizeMasterWinAmount);
            break;
        case CoinControlFeatures:
