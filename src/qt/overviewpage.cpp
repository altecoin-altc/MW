...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
	TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MW)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
	...
	CAmount MWAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

	// MasterWin Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // Combined balances
	CAmount availableTotalBalance = MWAvailableBalance; //+ matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance; // + zerocoinBalance;

											 // MW labels
	ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, MWAvailableBalance, false, BitcoinUnits::separatorAlways));
	...
	// MW Available
	bool showMWAvailable = settingShowAllBalances || MWAvailableBalance != nTotalBalance;
	...
	bool showWatchOnlyMWPending = showMWPending || watchUnconfBalance != 0;
	ui->labelPendingText->setVisible(showMWPending || showWatchOnlyMWPending);
	ui->labelUnconfirmed->setVisible(showMWPending || showWatchOnlyMWPending);
	ui->labelWatchPending->setVisible(showWatchOnlyMWPending && showWatchOnly);

	// MW Immature
	bool showMWImmature = settingShowAllBalances || immatureBalance != 0;
	bool showWatchOnlyImmature = showMWImmature || watchImmatureBalance != 0;
	ui->labelImmatureText->setVisible(showMWImmature || showWatchOnlyImmature);
	ui->labelImmature->setVisible(showMWImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

	// MW Locked
	bool showMWLocked = settingShowAllBalances || nLockedBalance != 0;
	bool showWatchOnlyMWLocked = showMWLocked || nWatchOnlyLockedBalance != 0;
	ui->labelLockedBalanceText->setVisible(showMWLocked || showWatchOnlyMWLocked);
	ui->labelLockedBalance->setVisible(showMWLocked || showWatchOnlyMWLocked);
	ui->labelWatchLocked->setVisible(showWatchOnlyMWLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
	...
	// update the display unit, to not use the default ("MW")
    updateDisplayUnit();
}
