...
CzmwTracker::CzmwTracker(std::string strWalletFile)
{
...
CzmwTracker::~CzmwTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzmwTracker::Init()
{
...
bool CzmwTracker::Archive(CMintMeta& meta)
{
...
bool CzmwTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzmwTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzmwTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzmwTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzmwTracker::GetSerialHashes()
{
...
CAmount CzmwTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzmwTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzmwTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzmwTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzmwTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzmwTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzmwTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzmwTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzmwTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzmwTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzmwTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmwTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzmwTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzmwTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzmwTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzmwTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzmwTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmw from DB\n", __func__, listDeterministicDB.size());
    }
