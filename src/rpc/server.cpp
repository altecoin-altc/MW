...
CAmount AmountFromValue(const UniValue& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 36000000.0)
...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop MasterWin server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "MasterWin server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        {"masterwin", "getcollateral", &getcollateral, true},
...
{
    return "> masterwin-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:33556/\n";
}
