...
CzmwWallet::CzmwWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmw seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMWSeed_deprecated()) {
                    LogPrintf("%s: Updated zmw seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmw seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmw wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzmwWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMWCount(nCountLastUsed);
    else if (!walletdb.ReadZMWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzmwWallet::Lock()
{
    seedMaster = 0;
}

void CzmwWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzmwWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzmwWallet::LoadMintPoolFromDB()
{
...
void CzmwWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzmwWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzmwWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmwTracker->HasPubcoinHash(pMint.first)) {
...
bool CzmwWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmwTracker which also adds to database
    pwalletMain->zmwTracker->Add(dMint, true);
...
void CzmwWallet::SeedToZMW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzmwWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzmwWallet::GenerateDeterministicZMW(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzmwWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzmwWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
