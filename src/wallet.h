...
enum AvailableCoinsType {
    ALL_COINS = 1,
    ONLY_DENOMINATED = 2,
    EXCEPT_MASTERNODE_COLLATERAL = 3,
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 MW at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zmw send
enum ZerocoinSpendStatus {
    ZMW_SPEND_OKAY = 0,                            // No error
    ZMW_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMW_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMW_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMW_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMW_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMW_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMW_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMW_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMW_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMW_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMW_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMW_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMW_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMW_SPENT_USED_ZMW = 14,                      // Coin has already been spend
    ZMW_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMW_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZMWBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMWOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmwAuto) const;
    ...
    */
    ...
    CzmwWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzmwTracker> zmwTracker;
    ...
    void setZWallet(CzmwWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmwTracker = std::unique_ptr<CzmwTracker>(new CzmwTracker(strWalletFile));
    }

    CzmwWallet* getZWallet() { return zwalletMain; }
    ...
    /** zmw reset */
    boost::signals2::signal<void()> NotifyzmwReset;
