...
#ifndef MasterWin_ZMWWALLET_H
#define MasterWin_ZMWWALLET_H
...
public:
    CzmwWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMW(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMW(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MasterWin_ZMWWALLET_H
